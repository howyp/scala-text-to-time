...
trait TextToDate extends RegexParsers {
  ...
  def positiveInteger: Parser[Int] = "\\d+".r ^^ (_.toInt) filter (_ > 0) withFailureMessage "positive integer expected"
  ...
  def daysBeforeAfter: Parser[Int] = opt(positiveInteger).map(_.getOrElse(1)) ~ "days?".r ~ opt("before"| "after") map {
